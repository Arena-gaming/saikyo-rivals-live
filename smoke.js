(()=>{
  ...
  function spawn(seedAge=0){
    ...
    const cfg=type==='body'?[8,13,.12,.25,.46,.86,.21,.35]:type==='veil'?[9,15,.18,.34,.70,1.25,.085,.17]:type==='wisp'?[6,11,.07,.16,.35,.72,.105,.20]:[4.5,8,.035,.075,.18,.42,.07,.145];
  ...
  function blob(p,x,y,r,a,stretch=1){const g=ctx.createRadialGradient(x-r*.12,y-r*.16,r*.02,x,y,r);g.addColorStop(0,`rgba(242,235,240,${a})`);g.addColorStop(.20,`rgba(231,220,229,${a*.80})`);g.addColorStop(.48,`rgba(211,201,212,${a*.43})`);g.addColorStop(.75,`rgba(191,184,199,${a*.16})`);g.addColorStop(1,'rgba(178,174,190,0)');ctx.save();ctx.translate(x,y);ctx.scale(1,stretch);ctx.fillStyle=g;ctx.beginPath();ctx.arc(0,0,r,0,6.283);ctx.fill();ctx.restore();}
  function draw(p){
    ...
    for(let j=0;j<lobes;j++){const ang=p.phase2+j*2.399+p.age*.035*(j%2?1:-1),off=r*(.12+.11*j/lobes);const rr=r*(j===0?1:(.38+.24*Math.sin(p.phase+j*1.7)**2));blob(p,p.x+Math.cos(ang)*off,p.y+Math.sin(ang)*off*.62,rr,a*(j===0?.78:.26),p.stretch*(j===0?1:.78+Math.random()*.06));}
    if(p.type==='wisp'||p.type==='thread'){ctx.save();ctx.translate(p.x,p.y);ctx.rotate(Math.sin(p.phase+p.age*.31)*.24);ctx.strokeStyle=`rgba(231,222,232,${a*.38})`;ctx.lineWidth=Math.max(.45,r*.035);ctx.lineCap='round';ctx.beginPath();ctx.moveTo(-r*.08,r*.18);ctx.bezierCurveTo(r*.42,-r*.05,-r*.52,-r*.52,r*(.35+.28*Math.sin(p.phase2)),-r*.92);ctx.stroke();ctx.restore();}
  }
  function frame(now){
    ...
    carry+=dt*(22*Math.max(.48,flow));while(carry>=1){spawn();carry-=1;if(Math.random()<.14)spawn();}
  ...
  function seed(){for(let i=0;i<120;i++){spawn();const p=particles[particles.length-1],age=Math.random()*p.life*.70;p.age=age;p.x+=p.vx*age;p.y+=p.vy*age-unit*.009*age*age;}}
